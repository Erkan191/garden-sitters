...
export async function POST(request) {
  try {
    ...
    if (reqRow.owner_id !== userId) {
      return Response.json(
        { error: "Only the owner can delete this request" },
        { status: 403 }
      );
    }
    ...
    if (offerDeleteErr) {
      return Response.json(
        { error: offerDeleteErr.message || "Failed to delete related offers" },
        { status: 400 }
      );
    }
    ...
    if (deleteErr) {
      return Response.json(
        { error: deleteErr.message || "Failed to delete request" },
        { status: 400 }
      );
    }
    ...
    if (!deletedRow) {
      return Response.json(
        { error: "This request could not be deleted. It may no longer be open." },
        { status: 400 }
      );
    }
