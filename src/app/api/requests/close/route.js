...
export async function POST(request) {
  try {
    ...
    if (reqRow.owner_id !== userId) {
      return Response.json(
        { error: "Only the owner can close this request" },
        { status: 403 }
      );
    }
    ...
    if (reqRow.status !== "open") {
      return Response.json(
        { error: "Only open requests can be closed" },
        { status: 400 }
      );
    }
    ...
    if (updateErr) {
      return Response.json(
        { error: updateErr.message || "Failed to close request" },
        { status: 400 }
      );
    }
    ...
    if (!updatedRow) {
      return Response.json(
        { error: "This request could not be closed. It may no longer be open." },
        { status: 400 }
      );
    }
