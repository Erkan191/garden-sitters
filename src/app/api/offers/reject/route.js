...
export async function POST(request) {
  try {
    ...
    if (reqRow.owner_id !== userId) {
      return Response.json(
        { error: "Only the owner can reject this offer" },
        { status: 403 }
      );
    }
    ...
    if (updateErr) {
      return Response.json(
        { error: updateErr.message || "Failed to reject offer" },
        { status: 400 }
      );
    }
    ...
    if (!updatedRow) {
      return Response.json(
        { error: "This offer could not be rejected. It may no longer be pending." },
        { status: 400 }
      );
    }
